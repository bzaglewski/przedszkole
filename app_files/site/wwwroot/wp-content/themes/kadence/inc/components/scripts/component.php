...
class Component implements Component_Interface {
	...
	public function action_enqueue_scripts() {
		...
		// Enqueue the Stickbits script.
		wp_register_script(
			'kadence-stickybits',
			get_theme_file_uri( '/assets/js/stickybits.min.js' ),
			array(),
			KADENCE_VERSION,
			true
		);
		wp_script_add_data( 'kadence-stickybits', 'async', true );
		wp_script_add_data( 'kadence-stickybits', 'precache', true );
		...
		$dependants = array();
		// if ( 'no' !== kadence()->option( 'header_sticky', 'no' ) ) {
		// 	$dependants[] = 'kadence-stickybits';
		// }
		wp_enqueue_script(
			'kadence-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			$dependants,
			kadence()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			true
		);
