...
class Component implements Component_Interface {
	...
	public function add_filter_for_add_to_cart_link( $html ) {
		add_filter( 'woocommerce_loop_add_to_cart_link', array( $this, 'filter_add_to_cart_link_link' ), 10, 3 );
		return $html;
	}
	...
	public function store_notice_body_classes( array $classes ) : array {
		if ( is_store_notice_showing() ) {
			...
			$classes[] = 'kadence-store-notice-placement-' . $placement;
			if ( 'above' === $placement ) {
	...
	public function my_account_body_classes( array $classes ) : array {
		if ( is_account_page() ) {
			$classes[] = 'kadence-account-nav-' . kadence()->option( 'woo_account_navigation_layout' );
		}
		return $classes;
	}
	/**
	...
	public function single_product_body_classes( array $classes ) : array {
		if ( is_product() ) {
			...
			$classes[] = 'kadence-cart-button-' . $size;
		}
		return $classes;
	}
	/**
	...
	public function check_for_fragment_support() {
		if ( $this->cart_in_header() ) {
			...
			if ( 'slide' === kadence()->option( 'header_cart_style' ) ) {
	...
	public function archive_loop_top() {
		...
		if ( kadence()->option( 'product_archive_show_results_count' ) || kadence()->option( 'product_archive_show_order' ) || kadence()->option( 'product_archive_toggle' ) ) {
