...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function filter_primary_nav_menu_dropdown_symbol( $title, $item, $args, int $depth ) {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			$title = '<span class="nav-drop-title-wrap">' . $title . '<span class="dropdown-nav-toggle" role="button" aria-pressed="false" aria-label="' . esc_attr__( 'Expand child menu', 'kadence' ) . '">' . kadence()->get_icon( 'arrow-down' ) . '</span></span>';
		}
	...
	public function filter_mobile_nav_menu_dropdown_symbol( $item_output, $item, int $depth, $args ) {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			$toggle_target_string = '#mobile-menu .menu-item-' . $item->ID . ' > .sub-menu';
