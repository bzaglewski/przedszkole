...
class Theme_Meta {
	...
	public function save_metabox( $post_id, $post ) {
		...
		if ( isset( $_POST['_kad_post_transparent'] ) ) {
			$trans_control_value = sanitize_text_field( wp_unslash( $_POST['_kad_post_transparent'] ) );
			update_post_meta( $post_id, '_kad_post_transparent', $trans_control_value );
		}
		if ( isset( $_POST['_kad_post_title'] ) ) {
			$title_control_value = sanitize_text_field( wp_unslash( $_POST['_kad_post_title'] ) );
			update_post_meta( $post_id, '_kad_post_title', $title_control_value );
		}
		if ( isset( $_POST['_kad_post_layout'] ) ) {
			$layout_control_value = sanitize_text_field( wp_unslash( $_POST['_kad_post_layout'] ) );
		...
		if ( isset( $_POST['_kad_post_content_style'] ) ) {
			$content_control_value = sanitize_text_field( wp_unslash( $_POST['_kad_post_content_style'] ) );
			update_post_meta( $post_id, '_kad_post_content_style', $content_control_value );
		}
		if ( isset( $_POST['_kad_post_vertical_padding'] ) ) {
			$padding_control_value = sanitize_text_field( wp_unslash( $_POST['_kad_post_vertical_padding'] ) );
